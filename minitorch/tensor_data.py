...
def broadcast_index(
    ...
    Returns:
        None
    """
    m, n = len(big_shape), len(shape)
    offset = m - n
    for d in range(offset, m):
        d_big_dim, d_small_dim = big_shape[d], shape[d - offset]
        if d_small_dim == 1:
...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    shape1 = tuple(reversed(shape1))
    shape2 = tuple(reversed(shape2))
    m = len(shape1)
    new_shape = []
    for i in range(m):
        a, b = shape1[i], shape2[i]
        if a == b:
            new_shape.append(a)
        elif a == 1 or b == 1:
